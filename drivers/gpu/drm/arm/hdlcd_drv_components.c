...
static int hdlcd_master_bind(struct device *dev)
{
	return component_add(dev, &hdlcd_ops);
}

static void hdlcd_master_unbind(struct device *dev)
{
	return component_del(dev, &hdlcd_ops);
}

static const struct component_master_ops hdlcd_master_ops = {
	//.add_components	= hdlcd_add_components,
	.bind		= hdlcd_master_bind,
	.unbind		= hdlcd_master_unbind,
};

static int compare_port(struct device *dev, void *data)
{
	struct device_node *np = data;
	pr_info("%s: dev %s [%s] to %s [%s]\n", __func__,
		dev->of_node->name, dev->parent->of_node->name, np->name, np->parent->name);
...
static int hdlcd_probe(struct platform_device *pdev)
{
	...
	return drm_of_component_probe(&pdev->dev, compare_port,
				      compare_encoder, &hdlcd_master_ops);
#endif
}
