				    ...
				    struct drm_plane_state *state)
{
	...
	if (state->rotation & ~(DRM_ROTATE_0 | DRM_ROTATE_180)) {
		DRM_DEBUG_KMS("Invalid rotation: %d\n", state->rotation);
		return -EINVAL;
	}
				      ...
				      struct drm_plane_state *state)
{
	...
	DRM_INFO("dest_h=%d, crtc_h=%d\n", dest_h, plane->state->crtc_h);

	if (plane->state->rotation == DRM_ROTATE_180) {
		pitch = -pitch;
		src_y = (drm_rect_height(&plane->state->src) >> 16) - src_y;
	}
	...
	hdlcd_write(hdlcd, HDLCD_REG_FB_LINE_PITCH, pitch);
...
static struct drm_plane *hdlcd_plane_init(struct drm_device *drm)
{
	...
	ret = drm_plane_create_rotation_property(plane, DRM_ROTATE_0,
