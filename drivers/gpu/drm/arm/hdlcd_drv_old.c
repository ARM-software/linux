...
static int hdlcd_load(struct drm_device *drm, unsigned long flags)
{
	...
	pr_info("%s canary 1 %s\n", __func__, dev_name(drm->dev));
...
static int hdlcd_drm_bind(struct device *dev, struct device *master, void *data)
{
...
static void
hdlcd_drm_unbind(struct device *dev, struct device *master, void *data)
{
...
static const struct component_ops hdlcd_ops = {
	.bind = hdlcd_drm_bind,
...
static int hdlcd_probe(struct platform_device *pdev)
{
	...
	return component_add(&pdev->dev, &hdlcd_ops);
#else
	int i;
	...
	pr_info("%s\n", __func__);
	/* parse the ports phandles and add them to the component list */
	for (i = 0; ; i++) {
		...
		component_add(&pdev->dev, &hdlcd_ops);
	}
#endif
}
...
{
	component_del(&pdev->dev, &hdlcd_ops);
	return 0;
}
...
/*
 * Display subsystem platform driver part
 */

static int arm_dss_bind(struct device *dev)
{
	struct drm_device *drm;
	struct hdlcd_drm_private *hdlcd;
	int err;
...
static void arm_dss_unbind(struct device *dev)
{
	struct drm_device *drm = dev_get_drvdata(dev);
	struct hdlcd_drm_private *hdlcd = drm->dev_private;
...
static const struct component_master_ops arm_dss_ops = {
	.bind		= arm_dss_bind,
	.unbind		= arm_dss_unbind,
};

static int compare_port(struct device *dev, void *data)
{
	struct device_node *np = data;
	pr_info("%s: dev %s [%p] to %s [%p]\n", __func__,
		dev->of_node->name, dev->of_node, np->parent->name, np->parent);
...
static int compare_encoder(struct device *dev, void *data)
{
	struct device_node *np = data;
	pr_info("%s: dev %s [%p] to %s [%p]\n", __func__,
		dev->of_node->name, dev->of_node, np->name, np);
	return dev->of_node == np;
}

static int arm_dss_probe(struct platform_device *pdev)
{
	return drm_of_component_probe(&pdev->dev, compare_port,
				      compare_encoder, &arm_dss_ops);
}

static int arm_dss_remove(struct platform_device *pdev)
{
	component_master_del(&pdev->dev, &arm_dss_ops);
	return 0;
}

static const struct of_device_id  arm_dss_of_match[] = {
	{ .compatible	= "arm,display-subsystem" },
	{},
};
MODULE_DEVICE_TABLE(of, arm_dss_of_match);

static struct platform_driver arm_dss_platform_driver = {
	.probe = arm_dss_probe,
	.remove = arm_dss_remove,
	.driver = {
		.name = "arm-dss",
		.of_match_table = arm_dss_of_match,
	},
};

static int __init hdlcd_drv_init(void)
{
	int err = platform_driver_register(&hdlcd_platform_driver);
	if (!err)
		err = platform_driver_register(&arm_dss_platform_driver);

	return err;
}

static void __exit hdlcd_drv_exit(void)
{
	platform_driver_unregister(&arm_dss_platform_driver);
	platform_driver_unregister(&hdlcd_platform_driver);
}

module_init(hdlcd_drv_init);
module_exit(hdlcd_drv_exit);
