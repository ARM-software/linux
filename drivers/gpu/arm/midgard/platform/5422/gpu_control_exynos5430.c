...
static int gpu_get_clock(struct kbase_device *kbdev)
{
	...
	if (IS_ERR(platform->fin_pll)  || (platform->fin_pll == NULL)) {
		...
		return PTR_ERR(platform->fin_pll);
	}
	...
	if (IS_ERR(platform->mout_g3d_pll)) {
		...
		return PTR_ERR(platform->mout_g3d_pll);
	}
...
int gpu_set_voltage(struct exynos_context *platform, int vol)
{
	...
	if (err) {
		GPU_LOG(DVFS_ERROR, "failed to set voltage, voltage: %d (err %d)\n",
				vol, err);
		return err;
	}
#endif /* CONFIG_REGULATOR */
...
int gpu_regulator_enable(struct exynos_context *platform)
{
	...
	if (err) {
		GPU_LOG(DVFS_ERROR, "failed to enable g3d regulator (err %d)\n",
				err);
		return err;
	}
	return 0;
...
int gpu_regulator_disable(struct exynos_context *platform)
{
	...
	if (err) {
		GPU_LOG(DVFS_ERROR, "failed to disable g3d regulator (err %d)\n",
				err);
		return err;
	}
	return 0;
...
int gpu_regulator_init(struct exynos_context *platform)
{
	...
	if (IS_ERR(platform->g3d_regulator)) {
		...
		GPU_LOG(DVFS_ERROR, "failed to obtain g3d regulator (err %d)\n",
				err);
	...
	if (err) {
		GPU_LOG(DVFS_ERROR, "failed to set g3d regulator operating voltage [%d] (err %d)\n",
				gpu_voltage, err);
		return err;
	}
